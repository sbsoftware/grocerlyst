...
describe PushNotifications do
  ...
  it "notifies subscribed existing list members when a new member joins" do
    ...
    PushNotifications.subscription_adapter.save(Crumble::Web::Push::Server::Subscription.new(session_id: "joining-session", web_push_subscription: WebPush::Subscription.new(endpoint: "https://example.com/joining", p256dh: "p256dh", auth: "auth")))
    ...
    ListAccessPermission.create(list_id: list.id, session_id: "joining-session", name: "Bob")

    sender.sessions.should eq(["owner-session"])
    sender.payloads.size.should eq(1)
    JSON.parse(sender.payloads.first)["body"].as_s.should eq("Bob joined Dinner")
  end
end
